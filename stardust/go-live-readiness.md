...
| B4 | **OpenGraph per template** (og:image + og:type + alt) | ✓ done | Added branded `default-meta-image.png` (fixes the 151 image-less og:image 404s) + a path-based `/metadata.json` sheet emitting `og:type` (website default; article for blog/academy/founder-chats + localized) and default `og:image:alt`. Edge-applied to all 1,033 pages, verified live. |
