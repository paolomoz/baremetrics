...
| # | Item | State | Notes |
|---|------|-------|-------|
| B1 | **Redirect map (old → new, 301)** | ✗ missing | Migration mirrored source URL paths, so most are 1:1 — but confirm URL parity across the full inventory and author 301s for any changed/consolidated/skipped slugs (incl. A4). Critical for preserving SEO at cutover. |
